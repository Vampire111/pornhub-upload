...
def make_category(title, pornhub_category):
    category = []
    for cat in pornhub_category:
        if cat[0][0] in title:
...
def make_tags(title, pornhub_tags):
    tags = []
    for tag in pornhub_tags:
        if tag in title:
            tags.append(tag)
